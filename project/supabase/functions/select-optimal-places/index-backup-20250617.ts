...
async function selectOptimalPlaces(
  ...
  // Get all places with user information first
  ...
  let normalizationResult = await getNormalizedPreferences(supabase, tripId, authHeader);
  
  // If normalization fails or returns no users, create a fallback normalization
  if (!normalizationResult || normalizationResult.normalizedUsers.length === 0) {
    console.warn('Normalization service returned no results, using fallback normalization');
    console.log('Available places for fallback:', allPlaces.length);
    
    // Get trip members for fallback normalization - get unique users from places
    const uniqueUserIds = [...new Set(allPlaces.map(p => p.user_id))];
    console.log('Unique user IDs from places:', uniqueUserIds);
    
    if (uniqueUserIds.length === 0) {
      throw new Error('No users found in places for this trip.');
    }
    ...
    if (usersError || !users || users.length === 0) {
      console.warn('Failed to fetch user details, using placeholder data');
      // Create placeholder users if database query fails
      const placeholderUsers = uniqueUserIds.map(userId => ({
        id: userId,
        email: `user-${userId.slice(0, 8)}@example.com`,
        display_name: `User ${userId.slice(0, 8)}`
      }));
      users.splice(0, 0, ...placeholderUsers);
    }
    
    // Create fallback normalization
    normalizationResult = {
      tripId,
      normalizedUsers: [],
      ...
      metadata: {
        ...
        edgeCaseHandling: ['fallback_normalization']
      }
    };
    
    // Group places by user and create simple normalization
    const placesByUser = allPlaces.reduce((acc, place) => {
      if (!acc[place.user_id]) acc[place.user_id] = [];
      acc[place.user_id].push(place);
      return acc;
    }, {} as Record<string, typeof allPlaces>);
    
    console.log('Places grouped by user:', Object.keys(placesByUser).map(userId => ({ userId, count: placesByUser[userId].length })));
    
    for (const user of users) {
      const userId = user.id;
      const userPlaces = placesByUser[userId] || [];
      
      if (userPlaces.length === 0) {
        console.warn(`No places found for user ${userId}`);
        continue;
      }
      
      const avgWishLevel = userPlaces.reduce((sum, p) => sum + p.wish_level, 0) / userPlaces.length;
      
      normalizationResult.normalizedUsers.push({
        userId,
        ...
        normalizedPlaces: userPlaces.map(place => ({
          ...
          fairnessScore: 1.0 / userPlaces.length,
          categoryWeight: 1.0
        })),
        userWeight: Math.sqrt(1 / userPlaces.length),
        fairnessContribution: 1.0
      });
    }
    
    console.log(`Fallback normalization complete: ${normalizationResult.normalizedUsers.length} users processed`);
  }
...
async function getNormalizedPreferences(supabase: any, tripId: string, authHeader?: string) {
  try {
    // Call the normalize-preferences function with proper auth
    ...
    const response = await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/normalize-preferences-v2`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        trip_id: tripId,
