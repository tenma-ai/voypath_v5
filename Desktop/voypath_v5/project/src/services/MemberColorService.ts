...
export class MemberColorService {
  ...
  /**
   * Get unified member colors for all views (uses Supabase real-time data)
   */
  static async getUnifiedMemberColors(tripId: string): Promise<{
    ...
    try {
      // Get member data with assigned colors
      const { data: membersWithColors } = await supabase
        .from('trip_members')
        .select(`
          user_id,
      ...
      if (!membersWithColors) {
        return { memberColors: {}, memberData: [] };
      }
      ...
      membersWithColors.forEach(member => {
        const color = this.getColorByIndex(member.assigned_color_index);
        if (color && member.users) {
          const userData = Array.isArray(member.users) ? member.users[0] : member.users;
          memberColors[member.user_id] = color.hex;
          memberData.push({
            userId: member.user_id,
      ...
      console.log('🎨 [MemberColorService] Unified colors for trip', tripId, ':', {
        memberColors,
        memberData
      });
      ...
      console.error('Error getting unified member colors:', error);
      return { memberColors: {}, memberData: [] };
    }
  }

  /**
   * Ensure all trip members have colors assigned
   */
  static async ensureAllMembersHaveColors(tripId: string): Promise<boolean> {
    try {
      // Auto-assign missing colors first
      await this.autoAssignMissingColors(tripId);
      
      // Validate and fix any issues
      const validation = await this.validateColorAssignment(tripId);
      if (!validation.valid) {
        console.log('🔧 [MemberColorService] Fixing color assignment issues for trip:', tripId);
        await this.fixDuplicateColors(tripId);
      }

      return true;
    } catch (error) {
      console.error('Error ensuring members have colors:', error);
      return false;
    }
  }
